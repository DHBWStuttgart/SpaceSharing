...
class TemplateckModelFont extends JModelList {
	...
	function __construct() {
		...
		$this->_path = JPATH_ROOT . '/administrator/components/com_templateck/fonts';
		$this->_data = null;
	}
	...
	function delete() {
		...
		if (count($cids)) {
			foreach ($cids as $cid) {
				...
				// delete the folder
				if (JFolder::exists($destpath . '/' . $fontname) && !JFolder::delete($destpath . '/' . $fontname)) {
	...
	function installFontsquirrel() {
		if (is_array(JRequest::getVar('file', '', 'files', 'array'))) {
			...
			require_once JPATH_ROOT . '/administrator/components/com_templateck/helpers/zip.php';
			...
			$app = JFactory::getApplication();
			$destpath = $this->_path . '/' . JFile::stripExt($filename);
	...
	function installFontgoogle() {
		...
		$fontfamily = str_replace('http://fonts.googleapis.com/css?family=', '', $fonturl);
