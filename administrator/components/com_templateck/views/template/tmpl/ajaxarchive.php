...
if ($saveintemplate == true) {
	...
	if (!JFolder::exists($defaulttemplatedest)) {
		...
		/*if (!JFolder::delete($defaulttemplatedest)) {
			$msg = '<p class="errorck">' . JText::_('CK_ERROR_DELETING_TEMPLATE_FOLDER') . '</p>';
		} else {
			$msg = '<p class="successck">' . JText::_('CK_SUCCESS_DELETING_TEMPLATE_FOLDER') . '</p>';
		}
		echo $msg;*/
		if (!JFolder::copy($defaulttemplatesrc, $defaulttemplatedest, '', true)) {
			$msg = '<p class="errorck">' . JText::_('CK_ERROR_CREATING_TEMPLATECOPY') . '</p>';
		} else {
			$msg = '<p class="successck" style="Font-size:14px;font-weight:bold;">' . JText::_('CK_SUCCESS_CREATING_TEMPLATECOPY') . '</p>';
		}
	}
	echo $msg;
} else {
