...
$loadBootstrap = (isset($blocs[0]->blocloadboostrap) && $blocs[0]->blocloadboostrap == '1') ? '1' : '0';
...
foreach ($blocs as $i => $bloc) {
	if (stristr($bloc->class, 'bannerlogo')) {
		...
		$logos_id[$bloc->ckid]->imgsrc = $bloc->blocbackgroundimageurl;
...
foreach ($logos_id as $id => $logo) {
	...
	$xmltext .= $openfield . " name=\"" . $id . "_logoimgsrc\" type=\"media\" default=\"" . $imageurl . "\" label=\"Logo image\" description=\"If you want to redirect the page on the logo click\" />\r\n";
